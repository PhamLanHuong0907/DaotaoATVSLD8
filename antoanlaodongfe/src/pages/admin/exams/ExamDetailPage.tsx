...
import { examTypeLabels, examModeLabels, questionTypeLabels, getUnifiedStatus, type StatusColor } from '@/utils/vietnameseLabels';
...
export default function ExamDetailPage() {
  ...
  const uni = getUnifiedStatus(
    exam.status,
    exam.scheduled_start || exam.scheduled_date,
    exam.scheduled_end,
  );
  ...
  return (
    <>
      ...
      {exam.questions.length === 0 ? (
        <Alert severity="info">Đề thi chưa có câu hỏi nào.</Alert>
      ) : (
        <Stack spacing={2}>
          {exam.questions
