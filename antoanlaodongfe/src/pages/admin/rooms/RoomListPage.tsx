...
import { useExamRooms, useDeleteExamRoom, useSubmitRoomForReview } from '@/hooks/useExamRooms';
...
import { examModeLabels, approvalStatusLabels, getUnifiedStatus, type StatusColor } from '@/utils/vietnameseLabels';
...
export default function RoomListPage() {
  ...
  // Fetch rooms without server status filter — all filtering is client-side now
  ...
  // Client-side Filter for Search, Mode, and Status
  const filteredRooms = useMemo(() => {
    ...
    return rawData.items.filter((r) => {
      ...
      const uni = getUnifiedStatus(r.approval_status, r.scheduled_start, r.scheduled_end);
      const matchStatus = status ? uni.label === status : true;
      return matchSearch && matchMode && matchStatus;
    });
  }, [rawData?.items, debouncedSearch, examMode, status]);

  const deleteMutation = useDeleteExamRoom();
  const [submitId, setSubmitId] = useState<string | null>(null);
  const submitMutation = useSubmitRoomForReview(submitId || '');

  const handleSubmitForReview = async (id: string) => {
    setSubmitId(id);
    try {
      await submitMutation.mutateAsync();
      enqueueSnackbar('Đã gửi duyệt phòng thi', { variant: 'success' });
    } catch (e) {
                ...
                {filteredRooms.map((r) => (
                  <TableRow key={r.id} sx={{ '&:hover': { bgcolor: 'action.hover' } }}>
                    ...
                    <TableCell align="center">
                      {(() => {
                        const uni = getUnifiedStatus(r.approval_status, r.scheduled_start, r.scheduled_end);
                        return <Chip size="small" label={uni.label} color={uni.color} />;
                      })()}
                    </TableCell>
                    <TableCell align="center">
                      {(() => {
                        const appSt = r.approval_status as ApprovalStatus;
                        const canSubmit = appSt === 'draft' || appSt === 'rejected';
                        return (
                          <>
                            {canSubmit && (
                              <Tooltip title="Gửi duyệt">
                                <IconButton size="small" color="info" onClick={() => handleSubmitForReview(r.id)}>
                                  <Send fontSize="small" />
                                </IconButton>
                              </Tooltip>
                            )}
