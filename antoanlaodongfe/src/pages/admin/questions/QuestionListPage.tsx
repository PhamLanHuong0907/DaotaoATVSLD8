...
import { Delete, CheckCircle, Visibility, FileUpload, FileDownload, ExpandMore } from '@mui/icons-material';
...
function OccupationQuestionSection({
  ...
  // 2. Fetch approved-only count (for Dashboard sync)
  const { data: approvedCountData } = useQuery({
    queryKey: ['questions-approved-count', occName],
    queryFn: () => questionApi.list({ occupation: occName, status: 'approved', page_size: 1 }),
    staleTime: 5 * 60 * 1000,
  });

  // 3. Fetch full list (lazy loaded)
  ...
  const approvedCount = approvedCountData?.total || 0;
  ...
  if (totalCount === 0 && !filters.search) return null;
  ...
  return (
    ...
    >
      <AccordionSummary expandIcon={<ExpandMore />} sx={{ bgcolor: 'grey.50', borderBottom: expanded ? '1px solid' : 'none', borderColor: 'divider' }}>
        ...
        <Chip label={`${totalCount} câu hỏi`} size="small" sx={{ ml: 2, fontWeight: 600 }} color="primary" variant="outlined" />
        <Chip label={`${approvedCount} đã duyệt`} size="small" sx={{ ml: 1, fontWeight: 600 }} color="success" variant="outlined" />
      </AccordionSummary>
...
export default function QuestionListPage() {
  ...
  const bulkApproveMutation = useMutation({
    mutationFn: (questionIds: string[]) => questionApi.bulkApprove({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['questions-count'] });
      qc.invalidateQueries({ queryKey: ['questions-full'] });
      enqueueSnackbar(`Đã duyệt thành công`, { variant: 'success' });
      setSelected(new Set());
    },
        ...
        <>
          {pagedOccs.map(occName => (
            <OccupationQuestionSection
              ...
              bulkApprove={(ids) => bulkApproveMutation.mutate(ids)}
            />
          ))}
