...
import { trainingGroupLabels, userRoleLabels } from '@/utils/vietnameseLabels';
...
export default function CourseFormPage() {
    ...
    const selectedOcc = useMemo(
        () => occupations?.find((o: any) => o.name === occupation),
        [occupations, occupation],
    );
    const availableSkillLevels = selectedOcc?.skill_levels || [1, 2, 3, 4, 5, 6, 7];
    ...
    const { data: reviewersData } = useQuery({
        ...
        queryFn: () => userApi.list({ role: 'manager', page_size: 100, is_active: true }),
    });
    const reviewerOptions = reviewersData?.items ?? [];
    ...
    const handleSubmit = () => {
        ...
        createMutation.mutate({
            ...
            assigned_reviewer_id: assignedReviewerId || undefined,
                ...
                <CardContent>
                    <Stack spacing={3} sx={{ maxWidth: 900 }}>
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid size={{ xs: 6, sm: 2 }}>
                                <TextField
                                    select fullWidth label="Bậc thợ" value={skillLevel}
                                    onChange={(e) => setSkillLevel(Number(e.target.value))}
                                >
                                    {availableSkillLevels.map((lv: number) => (
                            ...
                            <Grid size={{ xs: 12, sm: 3 }}>
                                <TextField
                                    select fullWidth label="Tài liệu nguồn" value={sourceDocIds.join(',')}
                                    onChange={(e) => setSourceDocIds(e.target.value ? e.target.value.split(',').filter(Boolean) : [])}
                                    helperText="Chọn tài liệu tham khảo (tuỳ chọn)"
                                >
                            ...
                            <Grid size={{ xs: 12, sm: 3 }}>
                                <TextField
                                    select fullWidth label="Người duyệt" value={assignedReviewerId}
                                    onChange={(e) => setAssignedReviewerId(e.target.value)}
                                    helperText="Chọn cán bộ quản lý sẽ duyệt khóa học (tuỳ chọn)"
                                >
                                    <MenuItem value="">— Chưa chọn —</MenuItem>
                                    {reviewerOptions.map((r: any) => (
                                        <MenuItem key={r.id} value={r.id}>{r.full_name} ({r.employee_id})</MenuItem>
                                    ))}
                                </TextField>
                            </Grid>
                        </Grid>
