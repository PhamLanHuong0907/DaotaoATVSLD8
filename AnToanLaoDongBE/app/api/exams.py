...
async def create_template(data: ExamTemplateCreate, user: User = Depends(require_staff())):
    create_data = data.model_copy(update={"created_by": str(user.id)}) if not data.created_by else data
    template = await exam_service.create_template(create_data)
...
async def generate_exam(data: ExamGenerateRequest, user: User = Depends(require_staff())):
    """Generate exam from approved template by selecting questions from bank."""
    create_data = data.model_copy(update={"created_by": str(user.id)}) if not data.created_by else data
    try:
        exam = await exam_service.generate_exam(create_data)
...
async def get_exam_detail(exam_id: str):
    ...
    return ExamDetailResponse(
        ...
        exam_period_id=str(period_id) if period_id else None,
        status=getattr(exam, "status", "pending_review"),
        questions=[
            ExamQuestionDetail(
