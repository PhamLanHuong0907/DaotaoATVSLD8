...
async def update_period(period_id: str, data: ExamPeriodUpdate) -> Optional[ExamPeriod]:
    ...
    if not period:
        return None
    # Block re-submitting when already at pending or approved stage
    if data.approval_status in (ApprovalStatus.PENDING_DEPT_REVIEW, ApprovalStatus.PENDING_REVIEW):
        if period.approval_status in (ApprovalStatus.APPROVED, ApprovalStatus.PENDING_DEPT_REVIEW, ApprovalStatus.DEPT_APPROVED, ApprovalStatus.PENDING_REVIEW):
            raise HTTPException(
                status_code=400,
                detail="Không thể gửi duyệt lại khi kỳ thi đã được duyệt hoặc đang chờ duyệt",
            )
...
async def create_room(data: ExamRoomCreate, created_by: str) -> ExamRoom:
    ...
    room = ExamRoom(
        ...
        approval_status=ApprovalStatus.PENDING_DEPT_REVIEW,
...
async def update_room(room_id: str, data: ExamRoomUpdate) -> Optional[ExamRoom]:
    ...
    # Block re-submitting when already at pending or approved stage
    if data.approval_status in (ApprovalStatus.PENDING_DEPT_REVIEW, ApprovalStatus.PENDING_REVIEW):
        if room.approval_status in (ApprovalStatus.APPROVED, ApprovalStatus.PENDING_DEPT_REVIEW, ApprovalStatus.DEPT_APPROVED, ApprovalStatus.PENDING_REVIEW):
            raise HTTPException(
                status_code=400,
                detail="Không thể gửi duyệt lại khi phòng thi đã được duyệt hoặc đang chờ duyệt",
            )
    update = data.model_dump(exclude_unset=True)
    update["updated_at"] = datetime.now(timezone.utc)
...
async def update_period_approval(
    ...
    if not period:
        return None
    if status == ApprovalStatus.APPROVED:
        if datetime.now(timezone.utc) >= period.start_date:
            raise HTTPException(400, "Không thể duyệt vì thời gian kỳ thi đã bắt đầu.")
    ...
    # Notify users in scope departments when period is approved
    if status == ApprovalStatus.APPROVED:
        try:
            from app.models.user import User
            from app.services.notification_service import create_bulk
            query: dict = {"is_active": True, "role": "worker"}
            if period.department_ids:
                query["department_id"] = {"$in": period.department_ids}
            users = await User.find(query).to_list()
            if users:
                await create_bulk(
                    user_ids=[str(u.id) for u in users],
                    type=NotificationType.EXAM_SCHEDULED,
                    title=f"Kỳ thi mới: {period.name}",
                    body=f"Từ {period.start_date.strftime('%d/%m/%Y')} đến {period.end_date.strftime('%d/%m/%Y')}",
                    link="/exams",
                )
        except Exception:
            pass
