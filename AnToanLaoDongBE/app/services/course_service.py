...
async def update_course_status(
    ...
    # Notify matched users when course is approved
    if status == ApprovalStatus.APPROVED:
        try:
            ...
            if course.occupation:
                query["occupation"] = course.occupation
            if course.skill_level:
                query["skill_level"] = course.skill_level
            if course.assigned_department_ids:
                query["department_id"] = {"$in": course.assigned_department_ids}
            users = await User.find(query).to_list()
            if users:
                await create_bulk(
                    user_ids=[str(u.id) for u in users],
                    type="new_course",
                    title=f"Khóa học mới: {course.title}",
                    body=f"Dành cho {course.occupation} — Bậc {course.skill_level}",
                    link="/study/courses",
                )
        except Exception:
            pass
