...
async def update_document_status(
    ...
    # Notify users in assigned departments when document is approved
    if status == ApprovalStatus.APPROVED and doc.assigned_department_ids:
        try:
            from app.models.user import User
            from app.services.notification_service import create_bulk
            users = await User.find(
                {"is_active": True, "role": "worker", "department_id": {"$in": doc.assigned_department_ids}}
            ).to_list()
            if users:
                await create_bulk(
                    user_ids=[str(u.id) for u in users],
                    type="new_document",
                    title=f"Tài liệu mới: {doc.title}",
                    body=doc.description or "",
                    link="/study/documents",
                )
        except Exception:
            pass
